...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return SCSolutionCoinGUI::tr("Received with");
    case RecvFromOther:
        return SCSolutionCoinGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return SCSolutionCoinGUI::tr("Sent to");
    case SendToSelf:
        return SCSolutionCoinGUI::tr("Payment to yourself");
    case Generated:
        return SCSolutionCoinGUI::tr("Mined");
    case RecvSCSolutionCoin:
        return SCSolutionCoinGUI::tr("Received scsolutioncoin");
    case SendSCSolutionCoin:
        return SCSolutionCoinGUI::tr("Sent scsolutioncoin");
    case Other:
        return SCSolutionCoinGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvSCSolutionCoin:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendSCSolutionCoin:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredSCSO, nCredSCSolutionCoin;
    wtx.GetCredit(nCredSCSO, nCredSCSolutionCoin, true);
    int64_t nCredit = nCredSCSO + nCredSCSolutionCoin;
    ...
    {
        if (nNet > 0 && nCredSCSolutionCoin > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvSCSolutionCoin, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendSCSolutionCoin, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvSCSolutionCoin;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv scsolutioncoin
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendSCSolutionCoin;
                    sub.address = CBitcoinAddress(ckidD).ToString();
