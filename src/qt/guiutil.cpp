...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitcoin URI
       if(!uri.isValid() || uri.scheme() != QString("scsolutioncoin"))
           return false;
       ...
       for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
       {
           ...
           else if (i->first == "amount")
           {
               if(!i->second.isEmpty())
               {
                   if(!BitcoinUnits::parse(BitcoinUnits::SCSO, i->second, &rv.amount))
                   {
                       return false;
                   }
               }
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("scsolutioncoin://"))
    {
        uri.replace(0, 16, "scsolutioncoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SCSolutionCoin.lnk";
}
...
{
    return GetAutostartDir() / "scsolutioncoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "[Desktop Entry]\n" \
                   ...
                   << "MimeType=x-scheme-handler/scsolutioncoin;\n";
        optionFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SCSolutionCoin") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  scsolutioncoin [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SCSolutionCoin"));
