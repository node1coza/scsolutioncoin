// Copyright (c) 2018 The Scsolutioncoin Developers
...
SCSolutionCoinGUI::SCSolutionCoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new SCSolutionCoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
...

{
    ...
    resize(1280, 720);
    setWindowTitle(tr("SCSolutionCoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/scsolutioncoin"));
    setWindowIcon(QIcon(":icons/scsolutioncoin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/scsolutioncoin/index.html");
#else
    QFile html("/opt/scsolutioncoin/index.html");
#endif
...
SCSolutionCoinGUI::~SCSolutionCoinGUI()
{
...
void SCSolutionCoinGUI::pageLoaded(bool ok)
{
...
void SCSolutionCoinGUI::urlClicked(const QUrl & link)
{
...
void SCSolutionCoinGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About SCSolutionCoin"), this);
    aboutAction->setToolTip(tr("Show information about SCSolutionCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for SCSolutionCoin"));
...
void SCSolutionCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void SCSolutionCoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("SCSolutionCoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/scsolutioncoin_testnet"));
            setWindowIcon(QIcon(":icons/scsolutioncoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/scsolutioncoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("SCSolutionCoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/scsolutioncoin_testnet"));
...
void SCSolutionCoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void SCSolutionCoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void SCSolutionCoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("SCSolutionCoin client"));
    trayIcon->setIcon(QIcon(":/icons/scsolutioncoin"));
...
#ifndef Q_OS_MAC
void SCSolutionCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SCSolutionCoinGUI::aboutClicked()
{
    AboutDialog dlg;
...
void SCSolutionCoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to SCSolutionCoin network", "", count));
}

void SCSolutionCoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void SCSolutionCoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void SCSolutionCoinGUI::changeEvent(QEvent *e)
{
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, nFeeRequired));
...
void SCSolutionCoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void SCSolutionCoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void SCSolutionCoinGUI::gotoSignMessageTab(QString addr)
{
...
void SCSolutionCoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void SCSolutionCoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void SCSolutionCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SCSolutionCoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void SCSolutionCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid SCSolutionCoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void SCSolutionCoinGUI::handleURI(QString strURI)
{
...
void SCSolutionCoinGUI::setEncryptionStatus(int status)
{
...
void SCSolutionCoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void SCSolutionCoinGUI::backupWallet()
{
...
void SCSolutionCoinGUI::changePassphrase()
{
...
void SCSolutionCoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void SCSolutionCoinGUI::lockWallet()
{
    if(!walletModel)
        return;
...
void SCSolutionCoinGUI::toggleLock()
{
    if(!walletModel)
        return;
...
void SCSolutionCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void SCSolutionCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SCSolutionCoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void SCSolutionCoinGUI::updateStakingIcon()
{
...
void SCSolutionCoinGUI::detectShutdown()
{
