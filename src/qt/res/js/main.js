...
var overviewPage = {
    init: function() {
        ...
        this.scsolutioncoinBal = $("#scsolutioncoinBal"),
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q=http://updates.scsolutioncoin.co/rss",
            dataType: 'jsonp'
        ...
        menu = [{
                 ...
                 {
                     name: ' About&nbsp;SCSolutionCoin...',
                     img: 'qrc:///icons/scsolutioncoin',
                     fun: function () {
    ...
    updateBalance: function(balance, scsolutioncoinBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            scsolutioncoinBal   = this.scsolutioncoinBal  .data("orig"),
            ...
            this.scsolutioncoinBal  .data("orig", scsolutioncoinBal),
        ...
        this.formatValue("scsolutioncoinBal",   scsolutioncoinBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. PzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" placeholder="Enter an SCSolutionCoin address (e.g. PzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="SCSolutionCoin"                    ' + (unit.type == 0 ? "selected" : "") + '>SCSO</option> \
                    <option value="1" title="Milli-SCSolutionCoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mSCSO</option> \
                    <option value="2" title="Micro-SCSolutionCoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;SCSO</option> \
                    <option value="3" title="Nano-SCSolutionCoins (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>nSCSO</option> \
                </select> \
            </div> \
...
function scsolutioncoinChatInit() {
    var menu = [{
            name: 'Send&nbsp;SCSolutionCoin',
            fun: function () {
                clearRecipients();
