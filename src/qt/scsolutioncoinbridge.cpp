#include "scsolutioncoinbridge.h"

#include "scsolutioncoingui.h"
#include "guiutil.h"
...
#include "scsolutioncoinbridge.moc"

SCSolutionCoinBridge::SCSolutionCoinBridge(SCSolutionCoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void SCSolutionCoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void SCSolutionCoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void SCSolutionCoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void SCSolutionCoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void SCSolutionCoinBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void SCSolutionCoinBridge::populateOptions()
{
...
// Transactions
bool SCSolutionCoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void SCSolutionCoinBridge::clearRecipients()
{
    recipients.clear();
}

bool SCSolutionCoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 OK, 1 SCSolutionCoin
        switch(rcp.txnTypeInd)
        {
            case TXT_OK_TO_OK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_OK_TO_ANON:
                formatted.append(tr("<b>%1</b> to SCSOLUTIONCOIN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to SCSOLUTIONCOIN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_OK:
                formatted.append(tr("<b>%1</b>, ring size %2 to SCSO %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsSCSolutionCoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your scsolutioncoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SCSO, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void SCSolutionCoinBridge::openAddressBook(bool sending)
{
...
void SCSolutionCoinBridge::openCoinControl()
{
...
void SCSolutionCoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void SCSolutionCoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap SCSolutionCoinBridge::listAnonOutputs()
{
...
void SCSolutionCoinBridge::populateTransactionTable()
{
...
void SCSolutionCoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void SCSolutionCoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString SCSolutionCoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void SCSolutionCoinBridge::populateAddressTable()
{
...
void SCSolutionCoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void SCSolutionCoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString SCSolutionCoinBridge::newAddress(bool own)
{
...
QString SCSolutionCoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void SCSolutionCoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool SCSolutionCoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool SCSolutionCoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void SCSolutionCoinBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void SCSolutionCoinBridge::appendMessage(int row) {
...
void SCSolutionCoinBridge::populateMessageTable()
{
...
void SCSolutionCoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool SCSolutionCoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool SCSolutionCoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString SCSolutionCoinBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool SCSolutionCoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool SCSolutionCoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void SCSolutionCoinBridge::connectSignals()
{
...
QVariantMap SCSolutionCoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap SCSolutionCoinBridge::listLatestBlocks()
{
...
QVariantMap SCSolutionCoinBridge::findBlock(QString searchID)
{
...
QVariantMap SCSolutionCoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap SCSolutionCoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap SCSolutionCoinBridge::txnDetails(QString blkHash, QString txnHash)
{
