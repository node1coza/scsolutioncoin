...
// /usr/include/boost/program_options/detail/config_file.h:163:17: error: call to function 'to_internal' that is neither visible in the template definition nor found by argument-dependent lookup
...
#else
    const char* pszModule = "scsolutioncoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SCSolutionCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SCSolutionCoin
    // Mac: ~/Library/Application Support/SCSolutionCoin
    // Unix: ~/.scsolutioncoin
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SCSolutionCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "scsolutioncoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong SCSolutionCoin will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("SCSolutionCoin"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
