...
void ThreadIRCSeed()
{
    ...
    for(;;)
    {
        ...
        if (fTestNet)
        {
            Send(hSocket, "JOIN #scsolutioncoinTEST\r");
            Send(hSocket, "WHO #scsolutioncoinTEST\r");
        } else
        {
            // randomly join #scsolutioncoin00-#scsolutioncoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #scsolutioncoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #scsolutioncoin%02d\r", channel_number).c_str());
        };
