...
public:
    ...
    int64_t GetSCSolutionCoinBalance() const;
    ...
    int64_t GetSCSolutionCoinDebit(const CTxIn& txin) const;
    int64_t GetSCSolutionCoinCredit(const CTxOut& txout) const;
    ...
    int64_t GetDebit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txin.IsAnonInput())
            {
                nDebit += GetSCSolutionCoinDebit(txin);
            } else
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetSCSolutionCoinCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nSCSO, int64_t& nSCSolutionCoin) const
    {
        nSCSO = 0;
        nSCSolutionCoin = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nSCSolutionCoin += GetSCSolutionCoinCredit(txout);
            } else
                nSCSO += GetCredit(txout);
            if (!MoneyRange(nSCSO)
                || !MoneyRange(nSCSolutionCoin))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableSCSolutionCoinCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableSCSolutionCoinCreditCached;
    
    mutable int64_t nCredSCSOCached;
    mutable int64_t nCredSCSolutionCoinCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableSCSolutionCoinCreditCached = false;
        
        nCredSCSOCached = 0;
        nCredSCSolutionCoinCached = 0;
        ...
        nAvailableSCSolutionCoinCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredSCSO, int64_t& nCredSCSolutionCoin, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredSCSO = nCredSCSolutionCoin = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredSCSOCached = 0;
            nCredSCSolutionCoinCached = 0;
            pwallet->GetCredit(*this, nCredSCSOCached, nCredSCSolutionCoinCached);
            fCreditSplitCached = true;
        };
        
        nCredSCSO = nCredSCSOCached;
        nCredSCSolutionCoin = nCredSCSolutionCoinCached;
        return true;
    }
    ...
    int64_t GetAvailableSCSolutionCoinCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableSCSolutionCoinCreditCached)
            return nAvailableSCSolutionCoinCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableSCSolutionCoinCredit() : value out of range");
            };
        };
        
        nAvailableSCSolutionCoinCreditCached = nCredit;
        fAvailableSCSolutionCoinCreditCached = true;
        return nCredit;
    };
